...
class TestRequest(unittest.TestCase):
    ...
    def test_parse_request_result(self):
        ...
        request = clusters_diff.Request('http://fakeurl.com', 'auth_key', 5.0)
        self.assertEquals(len(request._parse_request_result(raw_data)), 2)
        self.assertEquals(request._parse_request_result(raw_data)[0].name,
                          diffable_target_1.name)
        self.assertEquals(request._parse_request_result(raw_data)[0].ts_to_val,
                          diffable_target_1.ts_to_val)
        self.assertEquals(request._parse_request_result(raw_data)[1].name,
                          diffable_target_2.name)
        self.assertEquals(request._parse_request_result(raw_data)[1].ts_to_val,
...
class TestClustersDiff(unittest.TestCase):
    ...
    def test_compute_dissymmetries_pctls(self):
        ...
        for diffable_target_a, diffable_target_b in args3:
            ...
            self.assertEquals(dissymmetries[0].name, "target_2")
            self.assertEquals(dissymmetries[0].measures, [1, 1])
            self.assertEquals(dissymmetries[0].pctls,
                              collections.OrderedDict(
                                [(50, 1.0), (90, 1.0), (99, 1.0), (99.9, 1.0)]))
            self.assertEquals(dissymmetries[1].name, "target_1")
