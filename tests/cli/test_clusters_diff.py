...
class TestRequest(unittest.TestCase):
    ...
    def test_prepare(self):
        ...
        self.assertEqual(request._request.get_header(
    ...
    def test_parse_request_result(self):
        ...
        diffable_target_1 = clusters_diff.DiffableTarget(
            "target_1", {10: 0.1, 20: 0.2})
        diffable_target_2 = clusters_diff.DiffableTarget(
            "target_2", {60: 0.1, 70: 0.2})

        request = clusters_diff.Request('http://fakeurl.com', 'auth_key', 5.0)
        self.assertEqual(len(request._parse_request_result(data_json)), 2)
        self.assertEqual(request._parse_request_result(data_json)[0].name,
                         diffable_target_1.name)
        self.assertEqual(request._parse_request_result(data_json)[0].ts_to_val,
                         diffable_target_1.ts_to_val)
        self.assertEqual(request._parse_request_result(data_json)[1].name,
                         diffable_target_2.name)
        self.assertEqual(request._parse_request_result(data_json)[1].ts_to_val,
...
class TestHostResult(unittest.TestCase):
    ...
    def test_get_error_to_query(self):
        ...
        self.assertEqual({k: sorted(v) for k, v in error_to_queries.items()},
...
class TestClustersDiff(unittest.TestCase):
    ...
    def test_fetch_queries(self):
        ...
        with mock.patch('six.moves.urllib.request.urlopen') as urlopen:
            ...
            host_result = clusters_diff.fetch_queries(
                "host", "prefix", "auth_key", [
                    "query"], "-24hours", "-2minutes",
                "5.0", "0.01", lambda x: x)
            ...
            self.assertEqual(
                len(host_result.diffable_queries[0].diffable_targets), 2)
            self.assertEqual(host_result.name, "host")
            ...
            self.assertEqual(predicted_dt_1.ts_to_val,
                             diffable_targets[0].ts_to_val)
            self.assertEqual(predicted_dt_2.name, diffable_targets[1].name)
            self.assertEqual(predicted_dt_2.ts_to_val,
                             diffable_targets[1].ts_to_val)
    ...
    def test_fetch_queries_error(self):
        ...
        with mock.patch('six.moves.urllib.request.urlopen') as urlopen:
            ...
            self.assertEqual(
                len(host_result.diffable_queries[0].diffable_targets), 0)
            self.assertEqual(host_result.name, "host")
            self.assertEqual(host_result.diffable_queries[0].name, "query")
            self.assertEqual(host_result.query_to_error,
                             {'query': 'not found'})
    ...
    def test_compute_pctls(self):
        ...
        mesures = [1, 2, 4, 8, 9, 6, 2, 4, 6, 1, 4, 5,
                   8, 4, 6, 7, 1, 3, 4, 8, 6, 3, 4, 5, 8, 7, 2]
    ...
    def test_compute_dissymmetries_pctls(self):
        ...
        for diffable_target_a, diffable_target_b in args2:
            ...
            self.assertEqual(dissymmetries[0].pctls,
        ...
        for diffable_target_a, diffable_target_b in args3:
            ...
            self.assertEqual(dissymmetries[1].pctls,
                             collections.OrderedDict(
                [(50, 1.0), (75, 1.0), (90, 1.0), (99, 1.0), (99.9, 1.0)]))
    ...
    def test_outer_join_diffables(self):
        ...
        self.assertEqual(
            sorted(formated_result, key=_key),
