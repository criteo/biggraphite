...
class DiskCache(Cache):
    ...
    def _cache_get(self, metric_name):
        ...
        encoded_metric_name = bg_accessor.encode_metric_name(metric_name)
        with self.__env.begin(self.__metric_to_metadata_db, write=False) as txn:
            id_metadata_timestamp = txn.get(encoded_metric_name)

        if id_metadata_timestamp == self._EMPTY:
            return None, True

        if not id_metadata_timestamp:
            # cache miss
            return None, False

        # found something in the cache
        split = id_metadata_timestamp.split(self._METRIC_SEPARATOR, 2)
        ...
        # Update timestamp if diff is lower than half of the TTL
        if int(time.time()) > (int(timestamp_str) + int(self.__ttl / 2)):
    ...
    def repair(self, start_key=None, end_key=None, shard=0, nshards=1):
        ...
        with self.__env.begin(self.__metric_to_metadata_db, write=True) as txn:
            ...
            for key, value in cursor:
                ...
                metric = self._accessor.get_metric(key)
                value_no_timestamp = value.rsplit(self._METRIC_SEPARATOR, 1)[0]

                expected_value = None
                if metric:
                    expected_value = self.__value_from_metric(metric) \
                                         .rsplit(self._METRIC_SEPARATOR, 1)[0]

                if value_no_timestamp != expected_value:
                    logging.warning(
                        "Removing invalid key '%s': expected: %s cached: %s" % (
