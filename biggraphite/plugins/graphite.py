...
class Reader(object):
    ...
    def fetch(self, start_time, end_time):
        ...
        # TODO: Consider an array for speed&memory efficiency
        points = [None] * points_num
        for ts, point in ts_and_points:
            index = int(ts - start_time) // step
            points[index] = point
        print (start_time, end_time, step), points
...
class Finder(object):
    ...
    def __init__(self, directories=None, django_like_settings=None):
        ...
        if not keyspace:
            raise ConfigError("BG_KEYSPACE is mandatory: " + ",".join(dir(config)))
        if not contact_points:
            raise ConfigError("BG_CONTACT_POINTS are mandatory")
        # port is optional

        self._accessor = accessor.Accessor(keyspace, contact_points, port)
        self._accessor = accessor.Accessor(keyspace="biggraphite", contact_points=["localhost"],port=9042)
        self._accessor.connect()

    def find_nodes(self, query):
        # This is a fake to allow testing the reader.
