...
def glob_prefilter(metric_names, graphite_glob):
    ...
    Args:
      ...
      graphite_glob: Graphite glob pattern.

    Returns:
      List of metric names that may be matched by the provided glob.
    """
    dots = graphite_glob.count(".")
    literal_glob_components = [
        (index, component)
        for (index, component) in enumerate(graphite_glob.split("."))
        if not _is_graphite_glob(component)
    ]
...
def glob(accessor, graphite_glob):
    """Get Cassandra metrics and directories matching a Graphite glob.
