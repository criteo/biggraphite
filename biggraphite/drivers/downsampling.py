...
class Downsampler(object):
    ...
    def feed(self, metric, datapoints):
        ...
        if metric.name not in self._metric_aggregates:
            self._metric_aggregates[metric.name] = MetricAggregates(metric.metadata, self._capacity)

        # Sort points by increasing timestamp, because put expects them in order.
        return self._metric_aggregates[metric.name].update(metric.metadata, sorted(datapoints))
    ...
    def _update_raw(self, datapoints, precision):
        ...
        for timestamp, value in datapoints:
            ...
            if point_epoch > current_epoch:
                ...
                # If there no point was received for a long while,
