...
def main(args=None):
    ...
    host_result_1 = fetch_queries(opts.hosts[0], opts.auth_keys[0], queries, opts.From,
                                  opts.Until, opts.timeout_s, opts.threshold, pbar.update)
    pbar.finish()
    # host_result_2
    pbar = progressbar.ProgressBar(maxval=len(queries)).start()
    host_result_2 = fetch_queries(opts.hosts[1], opts.auth_keys[1], queries, opts.From,
                                  opts.Until, opts.timeout_s, opts.threshold, pbar.update)
    pbar.finish()
