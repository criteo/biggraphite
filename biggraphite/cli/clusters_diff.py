...
class Request(object):
    ...
    def _parse_request_result(self, raw_data):
        ...
        for json_object in json_data:
            ...
            ts_to_val = {}
            for datapoint in json_object['datapoints']:
                ts = datapoint[1]
                val = datapoint[0]
                ts_to_val[ts] = val
    ...
    def execute(self):
        ...
        try:
            ...
            diffable_targets = self._parse_request_result(raw_data)
        except IOError as e:
...
class HostResult(object):
    ...
    def add_error(self, query, e_msg):
        ...
        self.query_to_error[query] = "%s" % e_msg
...
class DiffableQuery(Diffable):
    ...
    def mesure_dissymmetries(self, other):
        ...
        for (diffable_targets1, diffable_targets2) in diffable_target_tuples:
            ...
            dissymmetry = self._mesure_non_equal_pts_percentage(val_dissymmetries, self.threshold)
            dissymmetries.append(dissymmetry)
        return dissymmetries
...
class TxtPrinter(Printer):
    ...
    def _print_percentiles(self, percentiles, prefix="", chip="", delay=""):
        ...
        for k in sorted(percentiles.pctls.iterkeys()):
            if prefix == "host":
                self._print("\t%s %spctl: %s s" % (delay, k, round(percentiles.pctls[k], 3)))
            else:
                self._print("\t%s %spctl: %s %%" % (delay, k, 100 * round(percentiles.pctls[k], 4)))
    ...
    def print_times(self, time_s_percentiles_list, query_to_time_s_list, verbosity, show_max):
        ...
        self._print("\n%s for : " % (Printer.pctls_kind_to_description["timing"]))
    ...
    for i, host in enumerate(hosts):
        ...
        for j, query in enumerate(queries):
            ...
            try:
                ...
                host_result.add_error(query, e.message)
                host_result.add_diffable_query(DiffableQuery(query, [], threshold))

            pbar.update(i*50+(j+1)*50/(len(queries)))
...
def compute_dissymmetries_pctls(diffables_a, diffables_b):
    ...
    if not diffables_a and not diffables_b:
        return [Percentiles("None", [0.0])]

    percentiles_list = []
    for a, b in _outer_join_diffables(diffables_a, diffables_b):
        if a is None or b is None:
            name = a.name if a is not None else b.name
...
def _outer_join_diffables(diffables_a, diffables_b):
    ...
    result = {name: (index_a.get(name), index_b.get(name)) for name in (
        index_a.viewkeys() | index_b.viewkeys())}
...
def _parse_opts(args):
    ...
    diff_parser.add_argument(
        '--no-printdiff', dest='printdiff',
        help='Do not print the diff returned by datadiff module', action='store_false')
...
def main(args=None):
    ...
    host_results = fetch_queries(
        opts.hosts, opts.auth_keys, queries, opts.From, opts.Until, opts.timeout_s, opts.threshold)
