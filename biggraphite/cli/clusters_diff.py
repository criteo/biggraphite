...
class _Worker():
    ...
    def compute_printable_queries_diff_list(self, fetched_queries_list):
...
class _Printer():
    ...
    def get_printable_report(self, report, chip="", delay=""):
        ...
        if report.avg_duration_per_host_dict and self._opts.timing:
    ...
    def get_printable_outputs(self, global_report, detailed_reports_per_query_list, detailed_reports_per_target_dict, printable_queries_diff_list):
        # TODO (t.chataigner) write datadiff result in a file
        seq = []
        if printable_queries_diff_list:
            for printable_query_diff in printable_queries_diff_list:
                seq.append("%s" % printable_query_diff)
            seq.append("")
...
def _parse_opts(args):
    ...
    diff_parser.add_argument('--printdiff', dest='printdiff', help='Print the diff returned by datadiff module', action='store_true')
    diff_parser.add_argument('--no-printdiff', dest='printdiff', help='Do not print the diff returned by datadiff module', action='store_false')
    parser.set_defaults(printdiff=False)

    diff_parser = parser.add_mutually_exclusive_group(required=False)
    diff_parser.add_argument('--timing', dest='timing', help='Print timing information', action='store_true')
    diff_parser.add_argument('--no-timing', dest='timing', help='Do not print timing information', action='store_false')
...
def main(args=None):
    ...
    printable_queries_diff_list = None
    if opts.printdiff:
        printable_queries_diff_list = _WORKER.compute_printable_queries_diff_list(fetched_queries_list)

    print(_PRINTER.get_printable_outputs(global_report, detailed_reports_per_query_list, detailed_reports_per_target_dict, printable_queries_diff_list))
