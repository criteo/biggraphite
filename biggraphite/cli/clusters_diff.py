...
def _parse_opts(args):
    ...
    comparison_params.add_argument('--input-file', metavar='FILENAME', dest='input_filename',
                                   ...
                                   required=True)
    comparison_params.add_argument('--from', metavar='FROM-PARAM', dest='from_param',
                                   action='store', default='-24hours',
                                   help='from param using Graphite API (default: %(default)s)')
    comparison_params.add_argument('--until', metavar='UNTIL-PARAM', dest='until_param',
                                   action='store', default='-2minutes',
                                   help='until param using Graphite API (default: %(default)s)')
    comparison_params.add_argument('--timeout', metavar='SECONDS', dest='timeout_s',
                                   action='store', help='timeout in seconds used to fetch queries \
                                   (default: %(default)ss)', type=float, default=5)
    comparison_params.add_argument('--threshold', metavar='PERCENT', action='store',
                                   help='percent threshold to evaluate relative equality between \
                                   two values (default: %(default)s%%)', type=float, default=1)
    ...
    outputs_params.add_argument('--output-file', metavar='FILENAME', dest='output_filename',
                                ...
                                default='')
    outputs_params.add_argument("-v", "--verbosity", action="count",
                                help='show more detailed outputs', default=0)
    outputs_params.add_argument("--show-max", metavar='N-LINES', dest='show_max',
                                help="truncate the number of shown dissymmetry in outputs \
                                (default: %(default)s)", type=int, default=5)
